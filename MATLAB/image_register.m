...
num_t = length(files_present);


base = (imread([Data_Folder, filesep, files_present(1).name]));
[yb,xb] = size(base);

%Loop through each time point, compare with base and register
for t = 1:num_t
    unregistered = (imread([Data_Folder, filesep, files_present(t).name]));
    ...
    disp(['Filename...', files_present(t).name, ' Time...', int2str(t), ' X offset...', num2str(xoff), ' Y offset...', num2str(yoff)]);
    ...
    register_by_offsets(unregistered, Result_Folder, files_present(t).name, xc, yc, xoffa, yoffa, xoff, yoff, t)
end
end
...
% Save images. If t =1, create tiff file, else append
if t == 1
